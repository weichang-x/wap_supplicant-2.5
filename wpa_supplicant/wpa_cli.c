...
char  id[8];
int wpa_cli_sunclound(int argc, char *argv[]);
void   add_network(char *devicename)
{
    char  deviceargs[256];
    memset(deviceargs,0,sizeof(deviceargs));
    strcat(deviceargs,"-i");
    strcat(deviceargs,devicename);

    char  *argv[]={"",deviceargs,"add_network"};
    memset(content,0,sizeof(content));
     wpa_cli_sunclound(3,argv);
     memset(id,0,sizeof(id));
     if(strlen(content)<=8)
        strcpy(id,content);
    
}
int   connect_wifiname(char *devicename,char  *wifiname)
{
    char  deviceargs[256];
    memset(deviceargs,0,sizeof(deviceargs));
    strcat(deviceargs,"-i");
    ...
    sprintf(name,"\'\"%s\"\'",wifiname);

    char  *argv[]={"",deviceargs,"set_network",id,"ssid",name};
    memset(content,0,sizeof(content));
    wpa_cli_sunclound(6,argv);
    if(strcmp(content,"OK") == 0)
        return  1;
    return  -1;
}
int    connect_password(char *devicename,char  *password)
{
    char  deviceargs[256];
    memset(deviceargs,0,sizeof(deviceargs));
    strcat(deviceargs,"-i");
    ...
    char  *argv[]={"",deviceargs,"set_network",id,"psk",passwd};
    memset(content,0,sizeof(content));
    wpa_cli_sunclound(6,argv);
    if(strcmp(content,"OK") == 0)
        return  1;
...
int    wifi_connect(char *devicename,char  *networkid)
{
    char  deviceargs[256];
    memset(deviceargs,0,sizeof(deviceargs));
    strcat(deviceargs,"-i");
    strcat(deviceargs,devicename);
    char  *argv[]={"",deviceargs,"select_network",networkid};
    memset(content,0,sizeof(content));
    wpa_cli_sunclound(4,argv);
    if(strcmp(content,"OK") == 0)
        return  1;
    return  -1;

}
int    wifi_disconnect(char *devicename,char  *networkid)
{
    char  deviceargs[256];
    memset(deviceargs,0,sizeof(deviceargs));
    strcat(deviceargs,"-i");
    strcat(deviceargs,devicename);
    char  *argv[]={"",deviceargs,"disconnect",networkid};
    memset(content,0,sizeof(content));
    wpa_cli_sunclound(4,argv);
    if(strcmp(content,"OK") == 0)
        return  1;
